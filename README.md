...
```
シャッフルと配列順の取り出し: 点群データ全体を一度シャッフルした後、指定したミニバッチサイズだけを配列順に取り出します。

効率化: ランダム関数を毎回の更新で使わないため、インデックス取得のオーバーヘッドが減ります。

重複回避: 全てのデータを取り出し終えた時（1エポック終了時）、再度データをシャッフルし直すことで、全ての点が均等に利用され、学習（位置合わせ）の偏りが避けられます。
